...
var cnItems = {
    ...
    'Added a button to the Heirlooms Window to see drop chances (or press C with the Heirloom window open). This opens up with chances at your current zone, and includes arrows to see other breakpoints you\'ve unlocked. Works with keyboard arrow keys as well!': '在传家宝界面中增加了一个按钮，可以用来查看传家宝掉率(也可以在传家宝界面中按C打开)。您可以在此看到已通过所有区域的传家宝掉率。',
    'You can now press K or click the button under settings to open up a list of all Hot(K)eys': '您可以按K，或者在设置界面下点击相应按钮来查看快捷键列表。',
