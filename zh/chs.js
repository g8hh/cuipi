...
var cnItems = {
    ...
    'List of maximum zones to Praid on Dailies corresponding to the list specified in Daily Praiding Z.  e.g. if Daily P raiding Z setting is 491,495 and this setting is 495,505, AT will P raid up to 495 from 491, and 505 from 495.  Set to -1 to always buy highest available prestige map.  If no corrsponding value, or value is invalid, defaults to max available (up to +': '在日常挑战中，根据日常重铸袭劫区域的数值。限制重铸袭劫地图的最大等级。例如：如果日常重铸袭劫区域设为 491,495 ，而此选项设为 495,505 ，那么脚本将会从区域491开始重铸袭劫，并且地图最大等级为495，然后从区域495开始，地图最大等级变为505。设为-1则永远制造最高等级的地图。如果两个选项数值对应不上，或者是数值不合法，则默认设为最高等级 (最高等级为超过区域',
    ...
    '0 to disable. Positive numbers are added to your Void Map zone. -1 for no cap.': '设为0则关闭此选项。设为正数则每次增加相应数值。设为-1则无上限。',
    ...
    'Example': '举例',
    'Void map zone=187 and This setting=10. New Voids run until 197).': '如果虚空区域为187，此项数值为10，则将运行区域187至区域197之间的新虚空地图。',
