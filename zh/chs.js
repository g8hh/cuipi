...
var cnItems = {
    ...
    'Sends trimps to fight if they\'re not fighting in the Toxicity and Nom Challenges, regardless of BAF. Essenitally the same as the one in combat, can use either if you wish, except this will only activate in these challenges (duh': '在毒性/美味挑战中，当脆皮没有在战斗时，自动让它们开始战斗，无视更好自动战斗的设置。跟战斗选项卡下的常时战斗效果类似，启用任意一个均可，但此选项只在相应挑战中生效。(不然呢？',
    ...
    'Delays buying armor prestige-upgrades during Want More Damage or Farming automap-modes, Although if you need health AND damage, it WILL buy armor prestiges tho. NOTE': '如果自动地图模式目前是需要更多伤害或者刷资源，则延迟购买护甲重铸升级。不过在需要更多伤害和生命值模式中仍然会购买护甲重铸升级。注意',
