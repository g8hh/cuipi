...
function selectChallenge(what) {
	...
	if (challengeSquaredMode) {
		...
		if (game.global.highestRadonLevelCleared >= 49){
			...
			desc += "%挑战<sup>" + number + "</sup>加成，并使挑战<sup><span class='icomoon icon-infinity'></span></sup>的加成增加" + prettify(totalReward * (1 + (rewards / 100))) + "%。";
		}
		else{
			desc += "%攻击力和生命值，并使氦获取量增加" + prettify(totalReward / 10) + "%。";
		}
...
function getScientistInfo(number, reward){
	switch (number){
		case 1: {
			const foreman = game.global.universe === 1 || !bwRewardUnlocked('Foremany') ? `、1工头` : ``;
			return reward ? `每周目将初始拥有5000科学点、100食物、100木头、10陷阱${foreman}` : 11500;
		}
		case 2: {
	...
	for (var x = 0; x < challengeList.length; x++){
		...
		if (lastMulti){
			if (highest > 0 || totalDif >= 1){
				if (totalDif >= 1){
					description += "<span class='greenText'>。如果您现在" + portalText + "，将使挑战<sup>" + number + "</sup>的加成增加" + prettify(totalDif) + "%，最终变为" + prettify(totalDif + reward) + "%。";
					if (game.global.highestRadonLevelCleared >= 49){
					...
					description += "<span class='redText'>。如果您现在" + portalText + "，挑战<sup>" + number + "</sup>的加成将保持在" + prettify(reward) + "%不变。</span>";
				}
			}
			else {
				description += "<span class='redText'>。您至少需要到达区域" + ((challenge.replaceSquareFreq) ? challenge.replaceSquareFreq : squaredConfig.rewardFreq) + "后才可以提升挑战<sup>" + number + "</sup>的加成，该加成目前为" + prettify(reward) + "%。</span>";
			}
		}
	}
	description += "</b>";
...
function manageEqualityStacks() {
	...
	let text = '已激活' + activeStacks + '层平等，它使我方脆皮';
...
var offlineProgress = {
	...
	getTimeOfflineText: function(){
		return '欢迎回来！您离线了' + this.formatTime(Math.floor(this.totalOfflineTime / 1000)) + '。' + this.getLimitBar();
	},
	...
	updateMapBtns: function() {
		...
		for (let x = 0; x < 4; x++) {
			...
			if (x === 4 && price > frags) {
				...
				mapText = `You can run <b>${this.mapsAllowed}张地图</b> while you wait!<br>Use ${this.mapsAllowed === 1 ? 'it' : 'them'} wisely...<br>您有${prettify(frags)}碎片。`;
				displayStyle = 'inline-block';
			}
	
			innerHTML = `区域${useWorld}地图<br>${prettify(price)}碎片<br>${this.countMapItems(useWorld)}个物品`;
	...
	updateBar: function(current) {
		...
		let newCellHTML = `格子${game.global.lastClearedCell + 2}`;
		let newZoneHTML = `区域${game.global.world}`;
		let newProgressTextHTML = `${prettify(current)} / ${prettify(this.progressMax)}个时刻(${width})`;
		...
		if (game.global.mapsActive) {
			...
			newMapDesc = `<span style='font-size: 0.8em'>运行地图：<i></i>${map.name}<i></i>(${map.level})<br/>格子${game.global.lastClearedMapCell + 2}<br/>剩余${this.countMapItems(map.level)}个物品</span>`;
		...
		let newExtraText = `在${prettify(Math.floor(timeSpent))}秒内处理了${prettify(current / 10)}秒的进度(每帧处理${this.loopTicks}次循环，速度为${prettify(speed)}倍)<br>预计完成时间：${this.formatTimeClock(remaining)}<br>${this.currentFluff}`;
...
function displaySelectedHeirloom(modSelected, selectedIndex, fromTooltip, locationOvr, indexOvr, fromPopup, fromSelect){
	...
	html += "，上限" + (prettify(allowed));
	html += "，剩余" + prettify(Math.max((allowed - spent), 0));
	html += "</span>";
	html += "<br/>";
...
function upgradeMod(confirmed, count){
	...
	if (!confirmed && game.options.menu.boneAlerts.enabled == 1) {
		tooltip('confirm', null, 'update', '您将花费' + prettify(cost) + resourceName + ((count > 1) ? '升级' + count + '次' : '升级') + '<i></i>' + game.heirlooms[heirloom.type][heirloom.mods[selectedMod][0]].name + '<i></i>。您确定要这么做吗？' , 'upgradeMod(true, ' + count + ')', 'Upgrade Mod');
		return;
	}
...
function setHeirRareText(forBones, forMain){
	...
	if (!forBones){
		...
		if (breakpoint == game.heirlooms.rarityBreakpoints.length) nextAt = "Max Rarity";
			...
			nextAt += "区域" + game.heirlooms.rarityBreakpoints[getUnbuffedHeirloomZoneBreakpoint()] + "后稀有度将提升";
		}
...
function setEmpowerTab() {
	...
	if (empowerTabDisplay === 'table-cell' && empowerMod) {
		...
		newNatureHTML = `<span class='${icons[empowerMod]}'></span>自然`;
	}
...
function updateGeneratorInfo() {
	...
	const totalMiText = `${prettify(game.global.magmite)}岩浆岩`;
...
function updateGeneratorUpgradeHtml(){
	...
	for (var item in game.permanentGeneratorUpgrades){
		...
		else text += `</br>${(upgrade.owned) ? "Done" : `${prettify(cost)}岩浆岩`}`
		elem.innerHTML = text;
		var state;
		if (upgrade.owned)
			state = "Done";
...
function setVoidCorruptionIcon(regularMap) {
	...
	let text = `该${regularMap ? '地图' : '虚空地图'}由于腐化开始变得不稳定。敌人的攻击力增加${prettify(attackScale)}倍，生命值增加${prettify(healthScale)}倍。`;

	if (!regularMap) {
		text += '该地图的氦获取量为本区域氦获取量(包含腐化格子的额外收益)的2倍！';
	}
		...
		for (let x = 0; x < cols; x++) {
			...
			if (maps) {
				...
				if (cell.empowerment) {
					...
					if (cell.empowerment === 'Poison') title = '毒';
					else if (cell.empowerment === 'Ice') title = '冰';
					else if (cell.empowerment === 'Wind') title = '风';
					title += `符记`;
...
function startFight() {
	...
	if (cell.name === 'Improbability' && game.global.spireActive) {
		...
		if (badCoord) badName += `<i></i>们(${prettify(badCoord)})`;
		displayedName = `<span id='actualBadName'>${badName}</span>`;
	} else if (badCoord && !displayedName) {
		const newName = cell.name.replace('_', ' ') + `<i></i>们(${prettify(badCoord)})`;
	...
	if (mutations.Living.active()) {
		...
		const tempName = cell.mutation ? mutations[cell.mutation].namePrefix + '<i></i>' + displayedName : displayedName;
		...
		badName = "<span class='badNameMutation u2Mutation' style='color: " + u2Mutations.getColor(cell.u2Mutation) + "'>" + u2Mutations.getName(cell.u2Mutation) + '<i></i>' + displayedName + '</span>';
...
function checkAverageExotics(which){
	...
	if (which == "Tauntimp"){
		...
		var text = "我方脆皮发现脆皮咚咚的数量不足，于是它们又繁育了" + needExotic + "只！新生的脆皮咚咚可以为" + prettify(addTrimps) + "名脆皮提供住房。"; 
...
function rewardLiquidZone() {
	...
	if ((addUniques || addRepeateds) && game.global.messages.Unlocks.enabled) {
		...
		text += `解锁了：<i></i>${unlockText.join('<i></i>，<i></i>')}<br/>`;
	}
	...
	if (messages.enabled && (messages.primary || messages.secondary)) {
		...
		if (messages.helium && heCount > 0) {
			resourceText.push(`氦 - ${prettify(heCount)}`);
		}
		if (messages.secondary) {
			resourceText.push(`脆皮上限 - ${prettify(game.resources.trimps.realMax() - initialResources.trimpsCount)}`);
			resourceText.push(`碎片 - ${prettify(game.resources.fragments.owned - initialResources.fragments)}`);
		}
		if (messages.primary) {
			resourceText.push(`食物 - ${prettify(game.resources.food.owned - initialResources.food)}`);
			resourceText.push(`木头 - ${prettify(game.resources.wood.owned - initialResources.wood)}`);
			resourceText.push(`金属 - ${prettify(game.resources.metal.owned - initialResources.metal)}`);
		}

		text += `获得资源：${resourceText.join('，')}<br/>`;
	}
	...
	if (trackedList && messages.exotic && messages.enabled) {
		text += `稀有脆皮：<i></i>${trackedList}<br/>`;
	}

	if (bones && messages.bone && messages.enabled) {
		text += `您发现了一只<i></i>${bones}<i></i>！<br/>`;
	}
...
function getSpireStory(spireNum, row, getAll){
	var spires = {
		...
		spire5: {
			...
			r7: "您慢慢地靠近，这个糟糕巨塔的顶部。您的恐惧变为自信，意志愈加坚固。幸好您有绒绒在邻，为您提供帮助。您和它会继续打拼，直到所有幻影消除。",
		...
		spire6: {
			r2: "攀爬尖塔时您听到了墙里传来的轻微呜咽声，德罗披提的幻影看起来状态可不怎么好。",
			r7: "尖塔中所有的颜色都消逝了，所有的生物只是黑白相间的僵尸而已。您真的只想料理掉这些怪物，然后尽快离开。",
			get r10(){
				var text = "您立于塔顶，往外远眺大地。";
				var obs = getObsidianStart();
				if (obs <= 800){
					var obsZones = obs - 700;
					text += "您觉得在远处还有一座尖塔矗立着，但" + obsZones + "个区域后的黑曜石墙阻止了您继续前进的脚步。";
					if (!Fluffy.checkU2Allowed()) text += "您暂时还不知道如何通过黑曜石墙。绒绒看起来很激动，似乎它想做点什么，但心有余而力不足。如果它再变强一些，或许……";
					else text += "绒绒表示可以熔化这堵墙，您需要将能量引入这个宇宙。时不我待，行动起来！";
				}
				else text += "一片焦土，破败不堪，但您可以看到在远处还有一座尖塔矗立着，没有什么可以阻挡您了。";
				return text;
			}
		},
		spire7: {
			r2: "这里的敌人仍然变异严重而十分强大，但尖塔看起来完全是徒有虚表。四周一片寂静，您有点颤抖，但仍然义无反顾。",
			get r10(){
				var text = "幻影弱到一个地步了，干掉它以后尖塔甚至没什么变化。尖塔前方有一片黑曜石荒原，没有任何值得注意的东西。这也算是好事吧，毕竟熔化黑曜石变得越来越难了。";
				var obs = getObsidianStart();
				if (obs < 810) text += "靠着绒绒的帮助，只要再努力一些，或许还可以再将黑曜石熔化一点。";
				else text += "尖塔通往前方区域的路已经开放了，但再往前的黑曜石墙已经无法熔化。"
				return text;
			}
		},
		spireU21: {
			r1: "污污向您表示祝贺，您成功迈出了尖塔漫漫征途的第一步。我方脆皮已经感到变强了一些！",
			r5: "孢子从自然敌人体内迸发出来，充斥了整个房间，我方脆皮齐刷刷咳嗽了起来。闷闷从上层发出了邪恶的笑声，但这笑声与其说让人畏惧，不如说让您感到想笑。在孢子和笑声的双重刺激下，您终于忍不住笑出声来，闷闷似乎恼羞成怒，向着更高层落荒而逃，只留下了它的脚步声。",
			r10: "污污表示，您目前进度只是总层数的十分之一再取十分之一。您考虑了一下是否要给它补上心心念念的分数乘法课，最后觉得这还不是合适的时机。",
			r15: "又一个自然敌人迸发完孢子退场了。我方脆皮显得淡定了许多，但战力还是实打实地下降了。污污提醒您，这一层只有两个自然敌人，都被您干掉了。尽快来到下一层摆脱这团云雾，我方脆皮的战斗力就能全数恢复了。",
			r50: "首层尖塔进度已过半了！闷闷没戏啦。",
			r90: "前方约10个格子处，您发现有个小型装置正在释放气体。污污告诉您，摧毁那个装置，就能从这个宇宙中永久抹去这一层，听起来绝对是好消息！",
			r100: "搞定了！第一层已经永远从宇宙中消失了。闷闷或许暂时还能溜掉，但只要您继续前进，在某条时间线上终究会把它逼入绝境。您正准备前往第二层时，发现了一个宝箱，里面装着四个突变传家宝！", //4 mutated heirlooms
			r105: "第一个自然敌人倒下了，但它们的数量明显多于上一层。污污表示自然突变本质上是一种活体生物，会随着攀登尖塔不断进化。这可真是太吓人了。",
			r120: "上层传来闷闷对污染的憎恶咆哮，但所谓的“自然”突变压根就不自然。",
			r140: "闷闷从刚被消灭的敌人残骸后冒了出来，一掌拍落某只茫然脆皮手中的匕首。它质问道您是否清楚锻造这把武器对星球的损害，您觉得没啥大不了的。它顿时语塞，悻悻然退回了尖塔深处。",
			r175: "矛盾的是，您有点赞同闷闷保护星球的观念，但这座尖塔造成的破坏远远超过您，更何况那货就是个讨厌鬼。",
			r190: "您要到达这一层的终点了，释放气体的小型装置再度出现。尽快将它摧毁！",
			r200: "又突破了一层，它也从宇宙中消失了。污污提醒道，传送后您可能就会遗失尖塔相关的记忆，但只要您需要，它会帮您想起来的。您获取了大量关于突变的知识，因此突变之种掉落数量永久翻倍。", //2x seeds
			r205: "自然突变还在变化！它到底会异变成什么玩意！？",
			r225: "尽管有着那些孢子毒云和突变，我方脆皮仍然精神振奋。只要它们能干掉敌人通常就会很开心。",
			r250: "您已经抵达总层数四分之一的位置。污污表示绒绒会为您骄傲的。",
			r275: "您忍不住一直在想，污污说过这里的突变是活体生物，它到底会变成什么样？！",
			r290: "您终于又在这层终点看到了那个释放气体的装置！您向污污询问道，那到底是什么东西，它表示那是一个尖塔稳定器。很明显，只有通过某种狂野的维度弯曲技术，尖塔和突变传播器才能存在，失去稳定器的同时它们也将消失。",
			r300: "您又搞定了！这个糟糕的地方已经消失了三层。看来污污每通过一层就变得更加强大，永久增加了50%的经验值获取量！", //+50% scruffy xp
			r306: "这里的突变似乎随着您攀登尖塔变得更像植物了。污污认为这就是闷闷要的效果。",
			r330: "闷闷再次出现，想要让您为自己对星球造成的破坏而感到羞耻。您基本无视了它，它再度悻悻离开了。",
			r360: "孢子跑到您脑子里挠痒痒了！",
			r390: "这层的尖塔稳定器终于映入眼帘了！您迫不及待想看看终点有什么好东西。",
			r400: "又通过一层，又摧毁了一个稳定器。距离尖塔中段只差一层了，您已经开始撼动这座尖塔的根基！在准备离开即将消失的这层时，您发现了一个奇怪的蓝色球体，自然而然地抓住了它。它对您的触碰没什么反应，但靠近附近的脆皮时却爆发出强烈的能量，使我方所有脆皮变得更强了。我方脆皮的暴击率永久增加10%！", //+10% crit
			r407: "突变还在蔓延……",
			r430: "您向污污询问道，如果有10队脆皮在尖塔中力竭而亡，究竟会发生什么，它表示是它把您传送出去的。您想知道原因，但它只让您别多问。",
			r460: "考虑了一下上次与污污的对话内容，您走向它，询问道如果您真的快干掉闷闷了，是否能暂停传送。它表示您必须干净利落地干掉闷闷。",
			r470: "您失去了所有脆皮！哦等等它们在这呢。",
			r490: "这一层也有稳定器，您也看到了它，完全没有出乎意料。",
			r500: "污污祝贺您摧毁了一半的闷闷尖塔！它想起闷闷恼火的样子就忍不住发笑。它笑了很久，终于停了下来。您向它询问道，您在这层终点找到的奇怪装置是什么。它按下一个按钮，将其绑在手臂上，并告诉您，从这座尖塔获得的攻击力、生命值和氡获取量加成都翻倍了！", //2x basic
			r507: "突变继续增长，正如污污的预料。它是不是开始有点像一棵树了？",
			r525: "您向污污询问，突变究竟是哪里来的。它说这一定是在它和绒绒弃暗投明后出现的，所以不太清楚，但那些种子里蕴含着极高的能量。",
			...
			r900: "您终于到最后一层了！污污表示最后一层是德罗披提亲手稳定的，无法永久从时间线上抹去，但至少这一周目您可以让它和闷闷一起消失。没有了孢子的干扰，您终于可以正常思考了。然后您发现脑中出现了新知识，突变之种掉落数量再度翻倍了！", //2x seeds
			r907: "您可以感觉到闷闷就在这一层的终点。那个小家伙走投无路，该给一切划上句号了。",
			r930: "一只脆皮走向您，递给您一株小树枝，它把这当成了幸运护符。有了这份力量，您必不可能输。",
			r960: "您离终点越来越近了，甚至几乎闻到了胜利的味道。或许那只是空气中孢子的味道。",
			r990: "正如污污的预料，这层终点并没有稳定器。但闷闷在那里，干掉它就足够了。",
			r1000: "闷闷发出一声尖叫，倒下了。您把变异传播器碾成了碎片，空气开始变得清新。然而在世界前方，您仍然可以看到大量突变肆虐。看来接下来您要去找醉醉了。污污表示，您对时间线做了非常了不起的贡献，尖塔的废墟就像灯塔一样，照亮了我方脆皮，使它们的攻击力、生命值，以及您的氡获取量变为原来的10倍，只要您不传送，本周目这个加成会持续存在。您为自己的丰功伟业感到自豪，继续前往下个区域。" //10x basic
		}
	}
...
function getDailyTopText(add){
	...
	for (var x = 0; x < 7; x++){
		...
		dayText = (usingScreenReader ? dayText : dayText.charAt(2))
		returnText += "'>周" + dayText;
		if (!dayDone){
	...
	returnText += "<div class='row' style='margin: 0'><div class='col-xs-6 lowPad dailyTop' style='font-weight: bold'>" + dayOfWeek(getDailyTimeString(add, false, true)) + " " + getDailyTimeString(add, true) + "</div><div class='col-xs-6 dailyTop lowPad' aria-live='off'>周" + dayOfWeek(getDailyTimeString(1, false, true)).charAt(2) + "的挑战内容在<span id='dailyResetTimer'>00:00:00</span>后重置</div></div>";
...
function getDailyChallenge(add, objectOnly, textOnly){
	...
	returnText += "</ul>日常挑战不会自动结束，完成后可<u><b>额外获得"  + prettify(getDailyHeliumValue(currentWeight, portalUni)) + "%</b></u>";
...
function getDailyRewardText(portalUni){
	var returnText = "";
	if (portalUni == 2){
		returnText += "氡，本次挑战中获得的传家宝价值的虚空物质";
		if (game.global.highestRadonLevelCleared >= 200) returnText += "，突变之种和污污经验值";
		else returnText += "和污污经验值";
	}
	else {
		...
		if (game.portal.Capable.locked == false) returnText += "和绒绒经验值";
	} 
	return returnText;
}
...
function fight(makeUp) {
	...
	if (game.global.soldierHealth <= 0) {
		...
		let msgText = `您的${prettify(game.resources.trimps.soldiers)}个脆皮${randomText}。`;
	...
	if (cell.health <= 0 || !isFinite(cell.health)) {
		...
		let killedText = `您${randomText}一个<i></i>${displayName}${challengeActive('Coordinate') ? '<i></i>集团' : ''}<i></i>！`;
		...
		if (game.global.mapsActive && cellNum === game.global.mapGridArray.length - 1) {
			...
			if (currentMapObj.name.search('仿生仙境') > -1 && allItemsEarned && game.options.menu.climbBw.enabled === 1 && game.global.repeatMap) {
...
function updateAntiStacks() {
	...
	if (game.global.antiStacks > 0) {
		...
		const verb = game.jobs.Amalgamator.owned > 0 ? '准备' : '繁殖';
		//const s = game.global.antiStacks === 1 ? '' : 's';
		elemText = makeIconEffectHTML("Anticipation", `我方脆皮花费了${game.global.antiStacks}秒来${verb}，因此可以多造成${number}%伤害。`, "icon-target2", "antiBadge", false, game.global.antiStacks)
	}
	if (elem && elem.innerHTML != elemText) elem.innerHTML = elemText;
}
	...
	expBreakdown: function (what) {
		...
		switch(what){
			...
			case "curious":
				...
				return;
			case "classy":
				elem.innerHTML = '绒绒开始获得经验值的区域。该数值初始为301，由于您有' + ((game.portal.Classy.modifier > 1) ? getPerkLevel("Classy") + "级上等特权" : "上等特权") + '，该数值减少了' + Math.floor(getPerkLevel("Classy") * game.portal.Classy.modifier) + '。';
				return;
			case "special":
			...
			case "knowledge":
				...
				return;
			case "ice":
				elem.innerHTML = '冰之启迪的加成。加成为(1 + (0.0025 × 冰赋能等级))，目前为' + prettify((1 + (0.0025 * game.empowerments.Ice.getLevel()))) + '。不影响最高绒绒经验值。'
				return;
			case "labs":
			...
			case "battlescruff":
				...
				return;
			case "mutators":
				elem.innerHTML = '突变因子的加成。目前为1.3倍。';
				return;
			case "frigid":
				elem.innerHTML = '寒冷挑战的加成。目前加成为' + prettify(game.challenges.Frigid.getTrimpMult()) + '倍。';
				return;
			case "stuffy":
				elem.innerHTML = "闷闷尖塔中知识的加成。目前加成为" + prettify(u2SpireBonuses.scruffyXp()) + "倍。";
		}
	},
...
function runEverySecond(makeUp) {
	...
	if (game.resources.helium.owned > 0 || game.resources.radon.owned > 0) {
		...
		const newHeliumPhHTML = `${prettify(heHr)}/小时`;
