...
var autoBattle = {
    ...
    items: {
        ...
        Tame_Snimp: {
            ...
            zone: 110,
            description: function(){
                return "可以使敌人中毒，持续10秒。使怒怒触发中毒的概率+" + prettify(this.poisonChance()) + "%，中毒伤害+" + prettify(this.poisonMod()) + "。如果敌人中毒了，则使它的攻击力-15%。";
            },
        ...
        Lich_Wraps: {
            ...
            zone: 113,
            description: function(){
                return "如果怒怒中毒、流血或震荡了，则使它的攻击力+" + prettify(this.attack()) + "，攻击间隔-15%，吸血+" + prettify(this.lifesteal() * 100) + "%，受到的伤害-" + prettify((1 - this.damageTakenMult()) * 100) + "%。";
            },
        ...
        Fearsome_Piercer: {
            ...
            zone: 165,
            description: function(){
                return "使怒怒的攻击力+" + prettify(this.attack()) + "，吸血+" + prettify(this.lifesteal() * 100) + "%，流血伤害+" + prettify(this.bleedMod() * 100) + "%，触发流血的概率+" + prettify(this.bleedChance()) + "%。如果怒怒装备了可以使敌人流血的物品，则使敌人在进入战斗前生命值上限-25%。"
            },
    ...
    getRingUi: function(){
        ...
        text += "</div><div class='ringContainer' style='text-align: center; padding-top: 2em;'><button class='btn btn-lg autoItemUpgrade' onclick='autoBattle.levelRing()' style='width: 90%'>升级灵戒！(花费" + prettify(this.getRingLevelCost()) + "晶块)</button><br/>";
    ...
    popup: function(updateOnly, statsOnly, itemsOnly, leaveMode, fromBattle){
        ...
        topText += prettify(this.dust) + "魔尘(每秒" + prettify(dustPs) + ")" + shardText + "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" + ((this.settings.practice.enabled == 1) ? "<b style='color: #921707'>Practicing</b>" : ((this.enemyLevel == this.maxEnemyLevel) ? ((this.canSeal) ? "刷怪中(等级已达上限)" : "需击杀" + (this.nextLevelCount() - this.enemiesKilled)) : "刷怪中")) + "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;敌人击杀数：" + this.sessionEnemiesKilled + "&nbsp;" + pctWon + "&nbsp;&nbsp;&nbsp;战斗失利数：" + this.sessionTrimpsKilled + "<br/>敌人等级" + this.enemy.level + ((this.profile) ? "(" + this.profile + ")" : "") + "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
        var buttons = "";

        if (!(updateOnly && statsOnly)) buttons = "<div id='abLevelButtons'><button id='abDecreaseLevel' onclick='autoBattle.levelDown()' class='btn-md btn auto noselect'>-降低敌人等级-</button><button onclick='autoBattle.toggleAutoLevel()' id='abAutoLevel' class='btn btn-md auto'>Set AutoLevel On</button><button onclick='autoBattle.levelUp()' id='abIncreaseLevel' class='btn btn-md auto'>+提高敌人等级+</button><button aria-label='Help' id='abHelpBtn' onclick='autoBattle.help()' class='icomoon icon-question-circle'></button><button aria-label='Close' id='abCloseBtn' onclick='cancelTooltip()' class='icomoon icon-close'></button></div>";
        ...
        for (var x = 0; x < things.length; x++){
            ...
            if (fighterName == "trimp"){
                if (this.items.Doppelganger_Signet.equipped){
                    ...
                    else statsText += "生命值：" + prettify(autoBattle.items.Doppelganger_Signet.doppHealth() - this.trimp.dmgTaken);
        ...
        if (this.popupMode == "items" || this.popupMode == "hidden"){
            ...
            for (x = 0; x < itemList.length; x++){
				...
				line += "<div class='autoItemContainer'><div tabindex=0 id='itemName" + item + "' class='autoItem autoItem" + equipClass + "' role=checkbox aria-checked='" + itemObj.equipped +  "' onclick='autoBattle.equip(\"" + item + "\")' onkeydown='autoBattle.hoverItem(\"" + item + "\", false, event)' onmouseover='autoBattle.hoverItem(\"" + item + "\")'>" + this.cleanName(item) + ((itemObj.noUpgrade) ? "" : "<i></i> 等级" + itemObj.level) + "</div>";
            ...
            for (var bonus in this.bonuses){
                ...
                var costText = prettify(cost) + "<i></i>" + ((bonusObj.useShards) ? "Shards" : "Dust");
                if (bonusObj.max && bonusObj.level >= bonusObj.max){
                    costTextColor = "red";
                    costText = "Max Level!"
                }
                costText = "<span id='" + bonus + "BonusPrice' class='" + costTextColor + "'>" + costText + "</span>";
                var maxLevel = (bonusObj.max) ? "最高" + bonusObj.max + "级": "Unlimited Purchases";
                text += "<button id='" + bonus + "BonusBox' onclick='autoBattle.buyBonus(\"" + bonus + "\")' class='autoBonusBox'>" + this.cleanName(bonus) + "<br/>等级：" + bonusObj.level + " - " + costText + "<br/>" + bonusObj.description() + "<br/>" + maxLevel + "</button>";
            }
            var oneCount = 0;
            var ownedItems = this.countOwnedItems();
            for (var oneTime in this.oneTimers){
            ...
            for (var x = 0; x < contracts.length; x++){
                ...
                else if (!this.activeContract) text += "<button onclick='autoBattle.acceptContract(\"" + item + "\")' class='btn btn-lg colorVoidy'>接受(" + prettify(this.contractPrice(item)) + "<i></i>" + this.getCurrencyName(item) + "<i></i>，通过一张区域" + itemObj.zone + "及以上区域的虚空地图)</button>";
...
var u2Mutations = {
    ...
    tree: {
        ...
        Poppin: {
            ...
            require: ['Heirmazing'],
            ring: 1,
            description: "突变区域中有5%概率额外产生1个新星敌人。",
            purchased: false
        },
        Worship2: {
            pos: [0, 18.5],
        ...
        AvgExotic: {
            ...
            check: function(){
                ...
                if (venimp + whipimp + magnimp > 0){
                    var text = "我方脆皮发现奇异外皮的数量不足，于是它们又繁育了一些！共繁育了" + venimp + "只脆皮爱神，" + whipimp + "只脆皮鞭者，" + magnimp + "只脆皮磁王。"
        ...
        Smashing: {
            ...
            color: '#00b700',
            require: ['Liq1'],
            description: "突变区域中有5%概率额外产生1个压缩敌人。",
            purchased: false
        },
        Liq2: {
            pos: [-7, -18.5],
