...
function tooltip(what, isItIn, event, textString, attachFunction, numCheck, renameBtn, noHide, hideCancel, ignoreShift) { //Now 20% less menacing. Work in progress.  (if this is 20% less menacing I'm terrified of what it looked like before -Q)
	...
	if (what == "Set Map At Zone"){
		...
		for (var x = 0; x < maxSettings; x++){
			...
			if (game.global.universe == 2 && game.global.highestRadonLevelCleared >= 32) presetDropdown += "<option value='10'" + ((vals.preset == 10) ? " selected='selected'" : "") + ">运行亚特兰蒂皮</option>";
			...
			if (game.global.universe == 2 && game.global.highestRadonLevelCleared >= 69) presetDropdown += "<option value='5'" + ((vals.preset == 5) ? " selected='selected'" : "") + ">运行黑之泥沼</option>";
			...
			tooltipText += "<div class='mazRx'><div style='text-align: center;'>X的次数</div><input aria-label='X times' value='" + vals.rx + "' type='number' id='mazRx" + x + "'/></div>";
			tooltipText += "<div class='mazBwWorld'><div style='text-align: center; margin-left: -0.5vw;'>仿生目标</div><input aria-label='Climb To' value='" + vals.bwWorld + "' type='number' id='mazBwWorld" + x + "'/></div>";
			...
			tooltipText += "<div class='mazTx'><div style='text-align: center;'>X的区域</div><input aria-label='X Zones' value='" + vals.tx + "' type='number' id='mazTx" + x + "'/></div>";
			tooltipText += "</div>"
		}
	...
	if (isItIn == "goldenUpgrades"){
		...
		if (what == "Helium" && game.global.runningChallengeSquared){
			...
			tooltipText += "<br/><br/><b class='red'>进行挑战<sup>" + cMode + "</sup>时您无法获得" + heliumOrRadon() + "！</b>";
		}
		costText = "免费";
	...
	if (what == "Mastery"){
		if (game.global.tabForMastery)
			...
			what = "Mutators"
			tooltipText = "<p>Click to view your Mutators.</p><p>您目前拥有<b>" + prettify(game.global.mutatedSeeds) + "</b>突变之种。</p>"
		}
	}
	...
	if (what == "The Magma"){
		...
		costText += "<div class='maxCenter'><div class='btn btn-info' id='confirmTooltipBtn' role=button tabindex=0 onclick='cancelTooltip()'>行</div></div>";
		game.global.lockTooltip = true;
		elem.style.left = "33.75%";
		elem.style.top = "25%";
	}
	...
	if (what == "MagnetoShriek"){
		...
		tooltipText = "您的小脆皮机器人似乎很擅长扭曲<i></i>" + plural + "<i></i>周围的磁场。每通过5个区域，您就可以激活它，使该区域<i></i>" + bossName + "<i></i>的攻击力减少" + shriekValue + "%。每次激活电磁尖啸后，都需要手动再次激活。";
		tooltipText += "<span id='roboTrimpTooltipActive' style='font-weight: bold'><br/><br/>";
		tooltipText += (game.global.useShriek) ? "电磁尖啸已经激活，将对下个<i></i>" + bossName + "<i></i>发射。" : "MagnetoShriek is NOT active and will not fire.";
		tooltipText += "</span>";
		costText = "";
		//elem.style.top = "55%";
	}
	...
	if (what == "Time Warp Limiter"){
		...
		costText = "<div class='maxCenter'><div class='btn btn-info' id='confirmTooltipBtn' role=button tabindex=0 onclick='cancelTooltip()'>搞定</div>";
	}
	if (what == "Scale Equality Scaling"){
		...
		costText = "<div class='maxCenter'><div class='btn btn-info' id='confirmTooltipBtn' role=button tabindex=0 onclick='cancelTooltip()'>搞定</div>" + spireBtn + "</div>";
		ondisplay = function(){
			verticalCenterTooltip();
		}
	}
	if (what == "Equality Scaling"){
		...
		if (game.global.buyAmt == "Max") what += "<i></i> X " + prettify(getPerkBuyCount(what));
		else if (game.global.buyAmt > 1) what += "<i></i> X " + prettify(game.global.buyAmt);
		what = what.replace("_", " ");
	}
	if (isItIn == "equipment"){
...
function getPsString(what, rawNum) {
	...
	if (game.global.challengeActive == "Frigid"){
		...
		currentCalc *= mult;
		textString += "<tr style='color: red'><td class='bdTitle'>Shattered (Frigid)</td><td class='bdPercent'>× " + prettify(mult) + "</td><td class='bdNumber'>" + prettify(currentCalc) + "</td></tr>";
	}
	...
	if (game.global.challengeActive == "Desolation" && what != "fragments"){
		...
		currentCalc *= mult;
		textString += "<tr style='color: red'><td class='bdTitle'>Desolation</td><td class='bdPercent'>× " + prettify(mult) + "</td><td class='bdNumber'>" + prettify(currentCalc) + "</td></tr>";
	}
...
function getBattleStatBd(what) {
	...
	if (game.global.challengeActive == "Archaeology" && what == "attack"){
		...
		var style = (count < 0) ? " style='color: red'" : "";
		currentCalc *= mult;
		textString += "<tr><td" + style + " class='bdTitle'>Relic Strength</td><td>×1.05</td><td>" + count + "</td><td class='bdPercent'>× " + prettify(mult) + "</td><td class='bdNumber'>" + prettify(currentCalc) + "</td>" + ((what == "attack") ? getFluctuation(currentCalc, minFluct, maxFluct) : "") + "</tr>";
	}
	...
	if (game.global.challengeActive == "Desolation" && what == "attack"){
		...
		currentCalc *= mult;
		textString += "<tr style='color: red'><td class='bdTitle'>Chilled (Desolation)</td><td>× 0.998</td><td>" + prettify(game.challenges.Desolation.chilled) + "</td><td class='bdPercent'>× " + prettify(mult) + "</td><td class='bdNumber'>" + prettify(currentCalc) + "</td>" + getFluctuation(currentCalc, minFluct, maxFluct) + "</tr>";

	}
	if (game.global.challengeActive == "Desolation" && what == "health"){
		mult = game.challenges.Desolation.trimpHealthMult(true);
		currentCalc *= mult;
		textString += "<tr style='color: red'><td class='bdTitle'>Chilled (Desolation)</td><td>× 0.999</td><td>" + prettify(game.challenges.Desolation.chilled) + "</td><td class='bdPercent'>× " + prettify(mult) + "</td><td class='bdNumber'>" + prettify(currentCalc) + "</td></tr>";
	}
	if (game.global.challengeActive == "Desolation"){
		mult = game.challenges.Desolation.desolationMult();
		currentCalc *= mult;
		textString += "<tr style='color: red'><td class='bdTitle'>Desolation</td><td>× " + (1 - game.challenges.Desolation.getReducePercent()).toFixed(3) + "</td><td>" + (game.global.world - 1) + "</td><td class='bdPercent'>× " + prettify(mult) + "</td><td class='bdNumber'>" + prettify(currentCalc) + "</td>";
	...
	if (what == "attack" && game.global.voidBuff && game.talents.voidMastery.purchased){
		currentCalc *= 5;
		textString += "<tr><td class='bdTitle'>Master of the Void</td><td>×5</td><td></td><td>× 5</td><td class='bdNumberSm'>" + prettify(currentCalc) + "</td>" + getFluctuation(currentCalc, minFluct, maxFluct) + "</tr>";
	...
	if (what == "attack" && game.global.universe == 1 && Fluffy.isActive() && game.talents.kerfluffle.purchased){
		...
		currentCalc *= amt;
		textString += "<tr><td class='bdTitle'>Kerfluffle</td><td>× 1.1</td><td>" + (Fluffy.getCurrentPrestige() + 1) + "</td><td>+ " + prettify((amt - 1) * 100) + "%</td><td class='bdNumberSm'>" + prettify(currentCalc) + "</td>" + getFluctuation(currentCalc, minFluct, maxFluct) + "</tr>"
	}
	//Cruffys
	...
	if (what == "health" && game.global.universe == 2 && u2Mutations.tree.Health.purchased)	{
		currentCalc *= 1.5;
		textString += "<tr><td class='bdTitle'>Healthy Mutator</td><td>× 1.5</td><td class='bdNumberSm'></td><td class='bdNumberSm'>× 1.5</td><td class='bdNumberSm'>" + prettify(currentCalc) + "</td></tr>";
	}
	...
	if (game.global.universe == 2 && game.global.novaMutStacks > 0 && what == "attack"){
		...
		currentCalc *= amt;
		textString += "<tr style='color: red'><td class='bdTitle'>Blinded (Nova)</td><td>× 0.99</td><td>" + game.global.novaMutStacks + "</td><td class='bdPercent'>× " + prettify(amt) + "</td><td class='bdNumber'>" + prettify(currentCalc) + "</td>" + getFluctuation(currentCalc, minFluct, maxFluct) + "</tr>";
	}
	if (game.global.universe == 2 && game.global.spireActive && !game.global.mapsActive && game.global.spireMutStacks > 0 && what == "attack"){
		...
		currentCalc *= amt;
		textString += "<tr style='color: red'><td class='bdTitle'>Spore Cloud (Spire)</td><td>× 0.99</td><td>" + game.global.spireMutStacks.toFixed(1) + "</td><td class='bdPercent'>× " + prettify(amt) + "</td><td class='bdNumber'>" + prettify(currentCalc) + "</td>" + getFluctuation(currentCalc, minFluct, maxFluct) + "</tr>";
	}
	...
	if (game.global.universe == 2 && what != "shield"){
		...
		if (cellCredit > 0){
			...
			currentCalc *= amt;
			textString += "<tr><td class='bdTitle'>Stuffy's Spire</td><td></td><td>" + cellCredit + "</td><td>× " + prettify(amt) + "</td><td class='bdNumberSm'>" + prettify(currentCalc) + "</td>" + ((what == "attack") ? getFluctuation(currentCalc, minFluct, maxFluct) : "") + "</tr>";
		}
	}
			...
			if (critChance > 1){
				...
				for (var check = 1; check < max; check++){
					if (critChance + checkAdd > check && critChance < (check + 2)){
						...
						textString += "<tr class='critRow'><td class='bdTitle'>" + getCritText(check + 1) + " Chance</td><td>" + (thisCritChance * 100).toFixed(1) + "%</td><td class='bdTitle'>" + getCritText(check + 1) + " Damage</td><td><span style='color: yellow;'>Crit!</span> × " + prettify(critMult) + "</td><td class='bdNumberSm'>" + prettify(critCalc) + "</td>" + getFluctuation(critCalc, minFluct, maxFluct) + "</tr>";
					}
				}
			}
...
function getLootBd(what) {
	...
	switch(what) {
		...
		case "Helium":
			...
			if (game.global.universe == 2 && game.global.glassDone && game.global.world > 175){
				...
				currentCalc *= mult;
				textString += "<tr><td class='bdTitle'>Advanced Processing (Glass)</td><td>× 1.1</td><td>" + (useGlassWorld - 175) + "</td><td>× " + prettify(mult) + "</td><td>" + prettify(currentCalc) + "</td></tr>";
			}
			if (game.global.universe == 2 && game.global.world >= 201){
				var mult = 400;
				if (u2Mutations.tree.AllRadon.purchased) mult *= 1.5;
				currentCalc *= mult;
				textString += "<tr><td class='bdTitle'>Mutated Zone</td><td>× " + mult + "</td><td></td><td>× " + mult + "</td><td>" + prettify(currentCalc) + "</td></tr>";
			}
	}
	...
	if (game.global.frigidCompletions > 0 && game.global.universe == 1 && what == "Helium"){
		...
		currentCalc *= amt;
		textString += "<tr><td class='bdTitle'>Frigid Completions</td><td>+ 2.5×挑战次数%</td><td>" + game.global.frigidCompletions + "</td><td>+ " + prettify((amt - 1) * 100) + "%</td><td>" + prettify(currentCalc) + "</td></tr>";
	}
	...
	if (game.global.challengeActive == "Hypothermia" && what == "Food/Wood/Metal"){
		...
		if (mult != 1){
			currentCalc *= mult;
			textString += "<tr style='color: red'><td class='bdTitle'>Hypothermia (Wood only)</td><td class='bdPercent'>×0.95</td><td>" + game.challenges.Hypothermia.embers + "</td><td>× " + prettify(mult) + "</td><td class='bdNumber'>" + prettify(currentCalc) + "</td></tr>";
		}
	}
	...
	if (game.global.universe == 2 && what == "Helium"){
		...
		if (cellCredit > 0){
			amt = u2SpireBonuses.basics();
			currentCalc *= amt;
			textString += "<tr><td class='bdTitle'>Stuffy's Spire</td><td></td><td>" + cellCredit + "</td><td>× " + prettify(amt) + "</td><td class='bdNumberSm'>" + prettify(currentCalc) + "</td></tr>";
		}
	}
...
function listWithAnd(list, replaceAnd){
	...
	for (var x = 0; x < list.length; x++){
		if (x == (list.length -1)) {
			if (list.length == 2) text += "<i></i>" + and + "<i></i>";
			else text += "<i></i>，<i></i>" + and + "<i></i>";
		}
		else if (x != 0) text += "<i></i>，<i></i>";
		text += list[x];
	}
	return text;
}
...
function addQueueItem(what) {
	...
	const name = multiplier > 1 ? `${baseName}<i></i> X${prettify(multiplier)}` : baseName;
...
function unlockMap(what) { //what here is the array index
	...
	if (game.options.menu.extraStats.enabled) elem.innerHTML = '<' + tagName + tooltip + ' class="' + btnClass + '" id="' + item.id + '" onclick="selectMap(\'' + item.id + '\')"><div class="onMapIcon"><span class="' + getMapIcon(item) + '"></span></div><div class="thingName onMapName">' + item.name + '</div><br/><span class="thingOwned mapLevel"><span class="stackedVoids">' + ((item.stacked) ? "(x" + (item.stacked + 1) + ") " : "") + '</span>等级' + level + abbrev + '</span><br/><span class="onMapStats"><span class="icomoon icon-gift2"></span>' + Math.floor(item.loot * 100) + '% </span><span class="icomoon icon-cube2"></span>' + item.size + ' <span class="icon icon-warning"></span>' + Math.floor(item.difficulty * 100) + '%</' +tagName +'>' + elem.innerHTML;
	else elem.innerHTML = '<' + tagName + tooltip + ' class="' + btnClass + '" id="' + item.id + '" onclick="selectMap(\'' + item.id + '\')"><span class="thingName">' + item.name + '</span><br/><span class="thingOwned mapLevel"><span class="stackedVoids">' + ((item.stacked) ? "(x" + (item.stacked + 1) + ") " : "") + '</span>等级' + level + abbrev + '</span></'+ tagName + '>' + elem.innerHTML;
	...
	function getWorldText(zoneNumber){
		...
		var universe2 = {
			...
			get w305(){
				...
				return "污污为您组织了一个惊喜派对，它甚至把所有" + prettify(game.resources.trimps.owned) + "只脆皮都藏在了沙发和其他东西后面，让您真正惊喜了一下，真是个好家伙！"
			},
			get w310(){
